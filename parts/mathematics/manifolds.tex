...
Let $x \in M_1$ be an element of the \hyperref[def:preimage]{preimage} of $U$; that is, $x$ is any point in $f^{-1}(U)$.
Since $U$ is open, there exists some $r = \epsilon > 0$ such that
$B_{\epsilon}(\text{element in } U) = B_{\epsilon}(f^{-1}(U)) \subseteq U$

\hyperref[def:continuous-mapping]{Continuity of $f$} implies that there exists $\delta > 0$ such atht