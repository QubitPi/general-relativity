...
\begin{equation}
    \underbrace{f\left( \frac{v}{V^2 - v^2} x - t \right)}_{Mathematics} = \underbrace{\varphi(v)\left( \frac{v}{V^2 - v^2} x - t \right)}_{Physics}
\end{equation}

which assumes taht transformation between $\tau$ and $t$ must be \textit{linear} which can be inferred by the
following fundamental postulate:
...
The postulate leaves with us the last question: \textit{How does homogeneity lead to linearity of Lorentz
transformation?}
